...
import struct, time, gc, bisect, umatrix, math, _thread, sys
...
#DECLARE ENCODERS
time.sleep(0.5)
Ejib2 = encoder(26,27,ch=5,sid=1,freq=80,mva=1)
Eboom = encoder(26,27,ch=6,sid=2,freq=80,mva=1)
Ejib1 = encoder(26,27,ch=7,sid=3,freq=80,mva=1)
Ecomp = encoder(26,27,ch=3,sid=5,freq=80,mva=1)    #COMP (1160 steps ish)
...
PAYLOAD_PACKET_REFRESH : int = 150 #ms between refreshes on decoded data and motor signals
POS_RECENT             : int = 0
...
while True:
    ...
    rxb = wait_packet(timeout_ms=150)
    IMU = lesing()
    #print(IMU)
    ...
    if rxb != 0:
        ...
        if time.ticks_diff(time.ticks_ms(),POS_RECENT) >= POS_REFRESH:                
                ...
                tilRC = '<' + str(save_boom_pos) + ' ' + str(save_jib_pos) + ' ' + str(save_slew) + ' ' + str(save_comp) + '>'
                pakkeRC = struct.pack('@20s',tilRC)
                uart1.write(pakke)
    ...
    send_packet(bytes([status[0], status[1] & 0x7F])+pakkeRC)

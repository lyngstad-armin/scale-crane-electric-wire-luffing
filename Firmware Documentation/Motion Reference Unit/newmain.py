...
while True:
    ...
    if absolutt_tid > 90000:
        absolutt_tid = time.ticks_ms()
    tilstring = '<'+str(round(roll_filtered, 3)) + ' ' + str(round(pos_filter[-1], 1)) + ' ' + str(hdt_uart) + ' ' + str(absolutt_tid) +'>'
    pakket_data = struct.pack("@30s", tilstring)
    print(struct.unpack("@30s", pakket_data))
